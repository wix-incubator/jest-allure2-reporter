...
Alternatively, you can [configure `jest-allure2-reporter/globals`](#TODO) to make them available globally.
...
URLs are built using [the template strings](#TODO) configured in the reporter options.
...
Advanced users may pass empty strings to `url` if they want to [build the URL dynamically](#TODO) via a config function:
...
Sets the [severity level](#TODO) of a test or suite.
...
Adds one or more [tags](#TODO) to a test or suite.
