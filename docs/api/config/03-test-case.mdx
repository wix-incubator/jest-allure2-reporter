...
const config = {
  testCase: {
    uuid: ({ package, file, test }) => `${package.name}:${file.path}:${test.fullName}`
  }
};
```
...
```typescript
ignored: ({ test }) => test.fullName.includes('WIP')
```
...
labels: {
  severity: 'critical',
  story: ({ test }) => `User Story: ${test.ancestorTitles[0]}`,
  custom: ({ file }) => `file:${file.path}`
}
```
...
parameters: {
  ...
  custom: ({ test }) => ({
    name: 'Test Depth',
    value: test.ancestorTitles.length,
    excluded: true
  })
}
```
...
const testCaseConfig: ReporterOptions['testCase'] = {
  uuid: ({ package, file, test }) => `${package.name}:${file.path}:${test.fullName}`,
  historyId: ({ test }) => `${test.identifier}:${test.title}`,
  displayName: ({ test }) => `[${test.identifier}] ${test.title}`,
  fullName: ({ file, test }) => `${file.path}::${test.fullName}`,
  description: ({ testCaseMetadata }) => `
    ## Test Case: ${testCaseMetadata.displayName}
    ${testCaseMetadata.description}
    \`\`\`js
