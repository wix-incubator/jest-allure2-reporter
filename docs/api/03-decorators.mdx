...
Understood. I'll focus on providing a more thorough and accurate reference for the `Attachment` and `FileAttachment` decorators based on the type definitions you've provided. Here are the revised sections:
...
1. `options`: An object of type `ContentAttachmentOptions`
   ...
   - `handler`: (Optional) A custom [attachment handler function](TODO) or its string alias
