...
Each test is composed of one or more steps, and each step can have its own [status](../features/statuses.mdx),
description, [attachments](attachments.mdx) and even nested steps.
...
There are several ways to create custom steps:

* `allure.step` function
* `allure.createStep` function
* `@Step` decorator
...
Custom steps can be nested, which allows you to create a tree-like structure of your tests.
See an example report to get an idea how tremendously it improves readability of any test.
