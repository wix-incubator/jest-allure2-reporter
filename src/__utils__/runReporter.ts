...
export async function runReporter(overrides: Partial<JestAllure2ReporterOptions>) {
  const options = {
    ...overrides,
    resultsDir: tempfile(''),
  };
...
async function readAllureResults(
  ...
  return await Promise.all(
    files.map(async (file) => {
      const content = await fs.readFile(path.join(allureResultsDirectory, file), 'utf8');
