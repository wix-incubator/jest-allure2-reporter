...
export const testRun: TestCaseCustomizer<TestRunExtractorContext> = {
  ...
  description: ({ testRunMetadata }) =>
    testRunMetadata.description?.join('\n\n') ?? '',
  ...
  parameters: compose2(
    custom.parameters({
      'Suites passed': ({ aggregatedResult }) =>
        aggregatedResult.numPassedTestSuites,
      'Suites failed': ({ aggregatedResult }) =>
        aggregatedResult.numFailedTestSuites,
      'Suites broken': ({ aggregatedResult }) =>
        aggregatedResult.numRuntimeErrorTestSuites,
      'Suites pending': ({ aggregatedResult }) =>
