...
export class TestFileSelectors {
  ...
  public start(testResult: TestResult) {
    return (
      ...
      this._services.time.getFileStartTime(testResult)
    );
  }

  public end(testResult: TestResult) {
    return (
      testResult.perfStats.end ?? this._services.time.getFileEndTime(testResult)
    );
  }
  ...
  public historyId(testResult: TestResult) {
    const relativePath = this._services.project.relative(
